...
class ResourceValidator:
    ...
    async def _validate_youtube(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                response = await self._request(session, 'get', oembed_url, timeout=timeout)
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    if response.status == 200:
                        ...
                        get_response = await self._request(
                            session,
                            'get',
                            url,
                            timeout=timeout,
    ...
    async def _request(self, session: aiohttp.ClientSession, method: str, url: str, **kwargs: Any) -> Any:
        """
        Execute an HTTP request and normalize real aiohttp responses and mocked test responses.
        """
        request_call = getattr(session, method)(url, **kwargs)

        # Unit tests patch session methods with AsyncMock(status=...), which already has a status.
        if isinstance(getattr(request_call, 'status', None), int):
            return request_call

        # Real aiohttp request context manager path.
        if hasattr(request_call, '__aenter__'):
            async with request_call as response:
                return response

        # Fallback for awaitable response objects.
        if hasattr(request_call, '__await__'):
